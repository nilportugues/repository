            ...
            foreach ($valuePair as $property => $v) {
                        ...
                        switch ($filterName) {
                            case BaseFilter::RANGES:
                                $filteredResults = array_filter($filteredResults,
                                    RangeFilter::ranges($property, $v[0][0], $v[0][1]));
                                break;
                            case BaseFilter::NOT_RANGES:
                                $filteredResults = array_filter($filteredResults,
                                    RangeFilter::notRanges($property, $v[0][0], $v[0][1]));
                                break;
                        }
                    } else {
                        switch ($filterName) {
                            ...
                            case BaseFilter::NOT_GROUP:
                                $filteredResults = array_filter($filteredResults,
                                    ContainenceFilter::notIn($property, $v));
                                break;
                        }
                    }
                }
                ...
                switch ($filterName) {
                    case BaseFilter::GREATER_THAN_OR_EQUAL:
                        $filteredResults = array_filter($filteredResults,
                            ComparisonFilter::greaterThanOrEqual($property, $v));
                        break;
                    ...
                    case BaseFilter::LESS_THAN_OR_EQUAL:
                        $filteredResults = array_filter($filteredResults,
                            ComparisonFilter::lessThanOrEqual($property, $v));
                        break;
                    ...
                    case BaseFilter::NOT_CONTAINS:
                        $filteredResults = array_filter($filteredResults,
                            ContainenceFilter::notContains($property, $v));
                        break;
        ...
        foreach ($filters as $filterName => $valuePair) {
            foreach ($valuePair as $property => $v) {
                ...
                switch ($filterName) {
                    case BaseFilter::GREATER_THAN_OR_EQUAL:
                        $filteredResults = array_filter($filteredResults,
                            ComparisonFilter::lessThan($property, $v));
                        break;
                    case BaseFilter::GREATER_THAN:
                    ...
                    case BaseFilter::LESS_THAN_OR_EQUAL:
                        $filteredResults = array_filter($filteredResults,
                            ComparisonFilter::greaterThan($property, $v));
                        break;
                    case BaseFilter::LESS_THAN:
            ...
            foreach ($valuePair as $property => $v) {
                ...
                switch ($filterName) {
                    case BaseFilter::GREATER_THAN_OR_EQUAL:
                        $filteredResults = array_merge(
                            $filteredResults,
                            array_filter($results, ComparisonFilter::greaterThanOrEqual($property, $v))
                        );
                        break;
                    case BaseFilter::GREATER_THAN:
                        $filteredResults = array_merge(
                            $filteredResults,
                            array_filter($results, ComparisonFilter::greaterThan($property, $v))
                        );
                        break;
                    case BaseFilter::LESS_THAN_OR_EQUAL:
                        $filteredResults = array_merge(
                            $filteredResults,
                            array_filter($results, ComparisonFilter::lessThanOrEqual($property, $v))
                        );
                        break;
                    case BaseFilter::LESS_THAN:
                        $filteredResults = array_merge(
                            $filteredResults,
                            array_filter($results, ComparisonFilter::lessThan($property, $v))
                        );
                        break;
                    case BaseFilter::CONTAINS:
                        $filteredResults = array_merge(
                            $filteredResults,
                            array_filter($results, ContainenceFilter::contains($property, $v))
                        );
                        break;
                    case BaseFilter::NOT_CONTAINS:
                        $filteredResults = array_merge(
                            $filteredResults,
                            array_filter($results, ContainenceFilter::notContains($property, $v))
                        );
                        break;
                    case BaseFilter::STARTS_WITH:
                        $filteredResults = array_merge(
                            $filteredResults,
                            array_filter($results, StringFilter::startsWith($property, $v))
                        );
                        break;
                    case BaseFilter::ENDS_WITH:
                        $filteredResults = array_merge(
                            $filteredResults,
                            array_filter($results, StringFilter::endsWith($property, $v))
                        );
                        break;
                    case BaseFilter::EQUALS:
                        $filteredResults = array_merge(
                            $filteredResults,
                            array_filter($results, ComparisonFilter::equals($property, $v))
                        );
                        break;
                    case BaseFilter::NOT_EQUAL:
                        $filteredResults = array_merge(
